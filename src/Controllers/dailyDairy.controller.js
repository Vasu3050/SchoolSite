...
import { DailyDiary } from "../Models/dailyDairy.model.js";
...
const writeNew = asyncHandler(async (req, res) => {
  ...
  if (grade && /^[A-Za-z]$/.test(grade)) {
    dataEntry.grade = grade.toLowerCase();
  }

  if (division && /^[A-Za-z]$/.test(division)) {
  ...
  if (expiresAt) {
    dataEntry.expiresAt = new Date(expiresAt);
  } else {
    dataEntry.expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
  }

  const diary = await DailyDiary.create(dataEntry);
...
const editDiary = asyncHandler(async (req, res) => {
  ...
  if (expiresAt) {
    editData.expiresAt = new Date(expiresAt);
  }
  if (grade) {
  ...
  const diary = await DailyDiary.findByIdAndUpdate(diaryId, editData, { new: true });
...
const deleteDiary = asyncHandler(async (req, res) => {
  ...
  const diary = await DailyDiary.findByIdAndDelete(diaryId);
...
const getDiary = asyncHandler(async (req, res) => {
  ...
  const diaryEntries = await DailyDiary.paginate(query, options);

  if (!diaryEntries || diaryEntries.docs.length === 0) {
    return res
      .status(200)
      .json(new ApiResponse(200, { diary: [] }, "No daily diary entries found for the specified criteria."));
  }

  res.status(200).json(
    new ApiResponse(200, {
      docs: diaryEntries.docs,
      pagination: {
        totalDocs: diaryEntries.totalDocs,
        limit: diaryEntries.limit,
        page: diaryEntries.page,
        totalPages: diaryEntries.totalPages,
        hasNextPage: diaryEntries.hasNextPage,
        hasPrevPage: diaryEntries.hasPrevPage,
        nextPage: diaryEntries.nextPage,
        prevPage: diaryEntries.prevPage,
      },
